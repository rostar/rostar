    ...
    if optimization == True:
        if sto == True:
            ...
            for i, F_w in enumerate( F_w_list ):
                ...
                res = minimize( residuum, args = [i, w_fitted, weight], x0 = [0.0180, .7], method = 'Nelder-Mead' )
                var1, var2 = res.x
...
sto_guessings = [0.02, 0.7]
ModelPlot = False
#Parameterliste anpassen bei det oder Sto
#STO 
paras = [[0.01257187, 0.5], ]
#Det
#paras = [[0.05], [0.1]]
...
if len( paras[0] ) > 1:
    ...
    sto = False
TensileTest( Dataplot, optimization, ModelPlot, paras, dataname_data, dataname_model, sto, pullout_test, plot_in_every_iter, ratio, E_f, r, m, sV0, Pf, n_int, weighting1_paras , weighting3_paras, det_tau_first_guessing, sto_guessings )
plt.xlim( 0, 8 )
plt.ylim( 0, .5 )
plt.show()
