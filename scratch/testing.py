...
if __name__ == '__main__':
    ...
    class Opt():
        ...
        def get_eps_extrap(self, t_scale, mtrx_shape, mtrx_scale):
        ...
        def get_eps(self, t_scale, mtrx_shape, mtrx_scale):
            tau = RV('weibull_min', shape=self.t_shape, scale=t_scale)
            random_field = RandomField(seed=True,
                                       lacor=5.,
                                        xgrid=np.linspace(0., self.length, 500),
        ...
        def residuum(self, params):
            ...
            mtrx_scale = params[2]
            try:
                eps_opt, sigma = self.get_eps(t_scale, mtrx_shape, mtrx_scale)
            ...
            print 'mtrx_shape', mtrx_shape
            print 'mtrx_scale', mtrx_scale
            ...
            if resid < self.smallest_resid:
                ...
                plt.plot(V1small.ex_type.eps_asc, V1small.ex_type.sig_c_asc, color='black')
                ...
                plt.draw()
            plt.show()
            return np.array([resid] * 3)

        def get_parameters(self):
            opt_params = fsolve(self.residuum, np.array([0.1, 17., 7.0]))
            print opt_params
    ...
    # filaments
        r = 0.00345
        tau = RV('weibull_min', shape=1.013, scale=.02066)
        Ef = 198e3
        Em = 26e3
        l = 1.0#RV('uniform', scale=6., loc=.5)
        theta = 0.0
        xi = 2.0#RV('weibull_min', scale=0.02, shape=5)
        phi = 1.
