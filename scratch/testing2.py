...
mr = np.array(
    [-0.023, 0.041, -0.031, -0.136, 0.35, -0.45, -0.43, -0.43, -0.486])
p = np.array(
    [0.002, 0.0025, -0.057, 0.0160, 0.025, -0.027, -0.074, -0.028, -0.029])
ft = np.array(
    [-0.085, 0.0082, -0.024, -0.019, -0.028, -0.026, -0.029, -0.026, -0.008])
cyc = np.array([3.1, 4.07, 3.55, 2.88, 4.12, 0.334, 0.36, 0.19, 0.227])
p2 = np.array([-0.0001, -0.00036, 0.00041, -0.00041, -0.00044,
               0.00046, -0.00017, -3.66e-5, 5.88e-5])
mr2 = np.array(
    [0.001, -0.0051, 0.012, 0.0295, 0.0193, -0.0156, 0.0066, 0.0445, 0.021])
ft2 = np.array([-0.00035, -4.37e-5, 0.00018, 0.00019,
                0.000265, -1.83e-5, -1.94e-5, -1.9e-5, -2.13e-5])
pmr = np.array(
    [-0.0069, 0.002, 0.0060, 0.0091, 0.00142, 0.0008, 0.0095, 0.0094, 0.0095])
pft = np.array([0.00073, 0.00026, -0.00045, 0.00041,
                0.00022, 0.000028, 0.00039, 0.00028, 0.00015])
mrft = np.array(
    [-0.00076, -0.0054, 0.001, -0.017, -0.015, 0.0076, -0.00082, 0.00079, 0.0003])
...
plt.plot(np.ones_like(full_p[-1]), full_p[-1],
         lw=0, marker='.', color='green', alpha=0.3)
plt.plot(1, yp[-1], label='pressure', color='green', marker='o', ms=8)

plt.plot(2 * np.ones_like(full_ft[-1]), full_ft[-1],
         lw=0, marker='.', color='blue', alpha=0.3)
plt.plot(2, yft[-1], label='firing times', color='blue', marker='o', ms=8)

plt.plot(3 * np.ones_like(full_mr[-1]), full_mr[-1],
         lw=0, marker='.', color='red', alpha=0.3)
plt.plot(3, ymr[-1], label='mix ratio', color='red', marker='o', ms=8)

plt.plot(4 * np.ones_like(full_p2[-1]), full_p2[-1],
         lw=0, marker='.', color='green', alpha=0.3)
plt.plot(4, yp2[-1], label='pressure$^2$', color='green', marker='o', ms=8)

plt.plot(5 * np.ones_like(full_ft2[-1]), full_ft2[-1],
         lw=0, marker='.', color='blue', alpha=0.3)
plt.plot(5, yft2[-1], label='firing times$^2$', color='blue', marker='o', ms=8)

plt.plot(6 * np.ones_like(full_mr2[-1]), full_mr2[-1],
         lw=0, marker='.', color='red', alpha=0.3)
plt.plot(6, ymr2[-1], label='mix ratio$^2$', color='red', marker='o', ms=8)

plt.plot(7 * np.ones_like(full_pft[-1]), full_pft[-1],
         lw=0, marker='.', color='green', alpha=0.3)
plt.plot(7, yp2[-1], label='pft', color='green', marker='o', ms=8)

plt.plot(8 * np.ones_like(full_mrp[-1]), full_mrp[-1] /
         100., lw=0, marker='.', color='blue', alpha=0.3)
plt.plot(8, ymrp[-1] / 100., label='mrp', color='blue', marker='o', ms=8)

plt.plot(9 * np.ones_like(full_ftmr[-1]), full_ftmr[-1],
         lw=0, marker='.', color='red', alpha=0.3)
...
def p_ft_func(p_array, ft_array):
    surface = np.exp(p_array * p[-1] + ft_array * ft[-1] + p_array **
                     2 * p2[-1] + ft_array**2 * ft2[-1] + p_array * ft_array * pft[-1])
    surface -= np.min(surface)
    return surface / np.max(np.abs(surface))


def p_mr_func(p_array, mr_array):
    surface = np.exp(p_array * p[-1] + mr_array * mr[-1] + p_array **
                     2 * p2[-1] + mr_array**2 * mr2[-1] + p_array * mr_array * pmr[-1])
    surface -= np.min(surface)
    return surface / np.max(np.abs(surface))


def ft_mr_func(ft_array, mr_array):
    surface = np.exp(ft_array * ft[-1] + mr_array * mr[-1] + ft_array **
                     2 * ft2[-1] + mr_array**2 * mr2[-1] + ft_array * mr_array * mrft[-1])
    surface -= np.min(surface)
    return surface / np.max(np.abs(surface))

# p_grid, ft_grid = np.mgrid[0:130.:1, 0:600:6]
# mlab.surf(p_grid, ft_grid, p_ft_func)
#
# mlab.show()

#p_grid, mr_grid = np.mgrid[0:150.:2, 0:10.:0.1]
#mlab.surf(p_grid, mr_grid, p_mr_func)

# mlab.show()

ft_grid, mr_grid = np.mgrid[0:600.:6, 0:10.:0.1]
mlab.surf(ft_grid, mr_grid, ft_mr_func)
...
plt.plot(np.linspace(0, 6000, 500),
         m[-1] / s[-1] * (np.linspace(0, 6000, 500) / s[-1])**(m[-1] - 1), color='black', lw=2)
plt.show()
