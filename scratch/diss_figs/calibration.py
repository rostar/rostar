...
class Model(HasTraits):
    ...
    def lack_of_fit(self, params):
        ...
        weights = np.linspace(10.0, 0.0, len(self.w))
        lack = np.sum(weights * (self.model_free(tau_shape, tau_scale, tau_loc) - self.interpolate_experiment) ** 2)
        print 'params = ', params
    ...
    def eval_params(self):
        ...
        params = minimize(self.lack_of_fit, np.array([0.109, 0.795]),
                          method='L-BFGS-B', bounds=((0.05, 1.), (0.01, 3.)))
        return params
