...
from interpolator import Interpolator2
...
class CB(HasTraits):
    ...
    interpolator = Instance(Interpolator2)
    position = Float
    x = Array
    ...
    def get_sigma_c_x(self, load):
        ...
        '''
        return self.interpolator.interpolator_mu_sigma_c(load, 0.0)

    def get_epsf_x_w(self, load):
        '''
        evaluation of matrix strain profile
        '''
        return self.interpolator.interpolator_mu_epsf(load, self.x)
    
    def get_epsm_x_w(self, load):
        '''
        evaluation of matrix strain profile
        '''
        return self.interpolator.interpolator_epsm(load, self.x)


class SCM2(HasTraits):
    ...
    def get_interpolator(self):
        ...
        return Interpolator2(CB_model = CB_model,
    ...
    def sort_cbs(self):
        ...
        if idx != 0:
            ...
            cbl = cb_list[idx - 1]
            cb = cb_list[idx]
            cbl.interpolator.Lr = (cb.position - cbl.position) / 2.
            cb.interpolator.Ll = cbl.interpolator.Lr
        else:
            # the new crack is the first from the left hand side
            cb_list[idx].interpolator.Ll = cb_list[idx].position

        if idx != len(cb_list) - 1:
            # there is a crack at the right hand side
            cb, cbr = cb_list[idx], cb_list[idx + 1]
            cbr.interpolator.Ll = (cbr.position - cb.position) / 2.
            cb.interpolator.Lr = cbr.interpolator.Ll
        else:
            # the new crack is the first from the right hand side
            cb_list[idx].interpolator.Lr = self.length - cb_list[idx].position
        ...
        for idx in idxs:
            mask1 = self.x_arr >= (cb_list[idx].position - cb_list[idx].interpolator.Ll)
            if idx == 0:
                mask1[0] = True
            mask2 = self.x_arr <= (cb_list[idx].position + cb_list[idx].interpolator.Lr)
    ...
    def sigma_m(self, load):
        ...
        if cb_load[0] is not None:
            for cb in cb_load:
                ...
                cb.interpolator.x_arr = self.x_arr[left:right] - cb.position
    ...
    def evaluate(self):
        ...
        while np.any(self.sigma_m(sigc_max) > self.matrix_strength):
            ...
            plt.plot(crack_position, sigc_min * self.E_m / (self.E_m * 0.95 + 200e3 * 0.05), 'ro')
...
#             plt.plot(self.matrix_strength)
#             plt.plot(self.sigma_m(sigc_min))
#             plt.show()
