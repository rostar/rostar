...
if __name__ == '__main__':
    ...
    def CB_composite_stress(w, tau, E_f, V_f, r, m, sV0, n_int):
        cb = CBRandXi()
        ...
        plt.legend(loc='best')
        plt.show()
    w = np.linspace(0, 1.2, 300)
    tau = 0.1#RV('weibull_min', shape=3., scale=.03)
    E_f = 200e3
    V_f = 0.01
    r = 0.01#RV('uniform', loc=0.001, scale=0.004)
    m = 7.
    ...
    #plt.legend()
    #plt.show()
    CB_composite_stress(w, tau, E_f, V_f, r, m, sV0, n_int)
